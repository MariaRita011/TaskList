...
//Classe das novas tarefas que serão adicionadas
class Task {
...
//Classe do array que irá armazenar as tarefas
class ArrList {
    constructor() {
        this.tasks = [];
    }

    //Ação de armazenar as tarefas no array
    addTask(task) {
        this.tasks.push(task)
    }

    //Ação de marcar a tarefa como "Feita"
    markTask(id) {
    ...
    //Ação de remover a tarefa
    removeTask(id) {
        this.tasks = this.tasks.filter((task) => task.id !== id);
    }
}

//Chamando nova instância do array
veryBigList = new ArrList();

//Função para criar uma nova tarefa
function createTask() {

    //Coletando o valor da tarefa
    const valor = document.getElementById('toDoInput').value;

    //Verificndo se o campo foi preencido
    if (valor == '') {
        document.getElementById("invalid").innerHTML = "Preencha o campo para adicionar"
    } else {

        //Verificando se o botão de "Editar" foi pressionado
        if (flag > 0) {
            ...
            //Atribuindo valores e limpando o campo 
        } else {
            const task = new Task(valor, generateId(), false);
...
//Função de dar valor único para cada ID
function generateId() {
    return Math.floor(Math.random() * 9999);
}


//Função de exibir as informações na tela
function renderTasks() {
    ...
    veryBigList.tasks.forEach(task => {
        ...
        <span class="task"id="${task.id}-title">${task.title}</span>
         <div id="botoes">
         <button id="${task.id}-button" class="btn" 
...
//Função para aparecer como marcada
...
function editTask(id) {
    ...
    document.getElementById('toDoInput').value = taskEdit.title
